...
void QSGRenderThread::processEvents()
{
    ...
    process_count++;
    if(process_count == 60*30){
        qCInfo(QSG_LOG_RENDERLOOP, QSG_RT_PAD, "--- begin processEvents()");
        process_count=0;
    }
    ...
    if(process_count == 0)
        qCInfo(QSG_LOG_RENDERLOOP, QSG_RT_PAD, "--- done processEvents()");
}
...
{
    qCInfo(QSG_LOG_RENDERLOOP, QSG_RT_PAD, "run()");
    ...
    while (active) {
        ...
        if (active && (pendingUpdate == 0 || !window)) {
            qCInfo(QSG_LOG_RENDERLOOP, QSG_RT_PAD, "done drawing, sleep...");
            sleeping = true;
    ...
    qCInfo(QSG_LOG_RENDERLOOP, QSG_RT_PAD, "run() completed");

...
#define qScreenEventInfo qInfo

#define Q_FORCE_SET_ALL_WINDOWS_WHEN_FOCUS_WINDOW_WITH_NULL
//#define Q_FORCE_SET_FIRST_WINDOW_WHEN_FOCUS_WINDOW_WITH_NULL
...
{
    T t;
    screen_get_event_property_pv(event, screen_traits<T>::propertyName,
...
bool QQnxScreenEventHandler::handleEvent(screen_event_t event, int qnxType)
{
    ...
    default:
        // event ignored
        qScreenEventInfo("unknown event %d", qnxType);
        return false;
    }
...
void QQnxScreenEventHandler::injectKeyboardEvent(int flags, int sym, int modifiers, int scan,
                                                 int cap)
{
    Q_UNUSED(scan);
    ...
    QString keyStr =
            (flags & KEY_SYM_VALID) ? keyString(sym, category) : capKeyString(cap, modifiers, key);

    qScreenEventInfo() << "focus window: " << QGuiApplication::focusWindow() << " "
                       << "allWindows: " << QGuiApplication::allWindows();

#ifdef Q_FORCE_SET_FIRST_WINDOW_WHEN_FOCUS_WINDOW_WITH_NULL
    QWindow *_test_window = QGuiApplication::focusWindow();
    if (_test_window == nullptr)
        _test_window = QGuiApplication::allWindows().count() <= 0
                ? nullptr
                : QGuiApplication::allWindows().first();
    QWindowSystemInterface::handleExtendedKeyEvent(_test_window, type, key, qtMod, scan, virtualKey,
                                                   modifiers, keyStr, flags & KEY_REPEAT);
#endif

#ifdef Q_FORCE_SET_ALL_WINDOWS_WHEN_FOCUS_WINDOW_WITH_NULL
    QWindow *_test_window = QGuiApplication::focusWindow();
    if (_test_window != nullptr)
        QWindowSystemInterface::handleExtendedKeyEvent(_test_window, type, key, qtMod, scan,
                                                       virtualKey, modifiers, keyStr,
                                                       flags & KEY_REPEAT);
    else {
        QWindowList windows = QGuiApplication::allWindows();
        for (auto window : windows) {
            QWindowSystemInterface::handleExtendedKeyEvent(window, type, key, qtMod, scan,
                                                           virtualKey, modifiers, keyStr,
                                                           flags & KEY_REPEAT);
        }
    }
...
#ifndef Q_FORCE_SET_FIRST_WINDOW_WHEN_FOCUS_WINDOW_WITH_NULL
#ifndef Q_FORCE_SET_ALL_WINDOWS_WHEN_FOCUS_WINDOW_WITH_NULL
    ...
    qScreenEventInfo() << "Qt key t=" << type << ", k=" << key << ", s=" << keyStr;
}

void QQnxScreenEventHandler::setScreenEventThread(QQnxScreenEventThread *eventThread)
{
    m_eventThread = eventThread;
    connect(m_eventThread, &QQnxScreenEventThread::eventsPending, this,
            &QQnxScreenEventHandler::processEvents);
}
...
void QQnxScreenEventHandler::processEvents()
{
    ...
    for (;;) {
        ...
        bool handled = dispatcher
                && dispatcher->filterNativeEvent(QByteArrayLiteral("screen_event_t"), event,
                                                 &result);
        if (!handled)
            handleEvent(event);
...
void QQnxScreenEventHandler::handlePointerEvent(screen_event_t event)
{
    ...
    if (qnxWindow != m_lastMouseWindow) {
        ...
        if (wOld) {
            ...
            qScreenEventInfo() << "Qt leave, w=" << wOld;
        }

        if (w) {
            QWindowSystemInterface::handleEnterEvent(w);
            qScreenEventInfo() << "Qt enter, w=" << w;
        }
    }
    ...
    // Convert buttons.
    // Some QNX header files invert 'Right Button versus "Left Button' ('Right' == 0x01). But they
    // also offer a 'Button Swap' bit, so we may receive events as shown. (If this is wrong, the fix
    // is easy.) QNX Button mask is 8 buttons wide, with a maximum value of x080.
    ...
    if (w) {
        ...
        if (m_lastGlobalMousePoint != globalPoint || m_lastLocalMousePoint != localPoint
            ...
            qScreenEventInfo() << "Qt mouse, w=" << w << ", (" << localPoint.x() << ","
        ...
        if (wheelDelta) {
            ...
            QWindowSystemInterface::handleWheelEvent(w, localPoint, globalPoint, wheelDelta,
                                                     Qt::Vertical);
            qScreenEventInfo() << "Qt wheel, w=" << w << ", (" << localPoint.x() << ","
...
void QQnxScreenEventHandler::handleTouchEvent(screen_event_t event, int qnxType)
{
    ...
    if (touchId < MaximumTouchPoints) {
        ...
        if (w) {
            ...
            m_touchPoints[touchId].area =
                    QRectF(w->geometry().left() + windowPos[0] - (touchArea[0] >> 1),
            ...
            qScreenEventInfo() << "Qt touch, w =" << w
                               << ", p=" << m_touchPoints[touchId].area.topLeft() << ", t=" << type;
        }
    }
}
...
void QQnxScreenEventHandler::handleDisplayEvent(screen_event_t event)
{
    ...
    if (screen_get_event_property_pv(event, SCREEN_PROPERTY_DISPLAY, (void **)&nativeDisplay)
        != 0) {
    ...
    if (!screen) {
        if (isAttached) {
            ...
            qScreenEventInfo("creating new QQnxScreen for newly attached display");
        ...
        if (!screen->isPrimaryScreen()) {
            ...
            qScreenEventInfo("removing display");
...
void QQnxScreenEventHandler::handlePropertyEvent(screen_event_t event)
{
    ...
    errno = 0;
    screen_window_t window = 0;
    if (Q_UNLIKELY(screen_get_event_property_pv(event, SCREEN_PROPERTY_WINDOW, (void **)&window)
                   != 0))
    ...
    default:
        // event ignored
        qScreenEventInfo() << "Ignore property event for property: " << property;
    }
}
...
void QQnxScreenEventHandler::handleKeyboardFocusPropertyEvent(screen_window_t window)
{
    ...
    if (Q_UNLIKELY(window
                   && screen_get_window_property_iv(window, SCREEN_PROPERTY_FOCUS, &focus) != 0))
...
void QQnxScreenEventHandler::handleGeometryPropertyEvent(screen_window_t window)
{
    ...
    qScreenEventInfo() << qtWindow << "moved to" << rect;
}

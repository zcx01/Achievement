...
if __name__ == "__main__":
    ...
    parse.add_argument('-f', '--fristMatrix',
                       help='比较dbc(矩阵),比较的dbc目录(文件),没有指定就使用配置文件中的路径,', default="")
    parse.add_argument('-t', '--twoMatrix', help='比较dbc（矩阵）,被比较的文件,也是旧的一方的文件')
    ...
    parse.add_argument('-d', '--dbcPath', help='dbc的目录，只输入-d，表示从-d复制枚举到配置文件中dbc中')
    ...
    if '-d' in sys.argv and '-r' in sys.argv and '-t' in sys.argv:
        ...
        if os.path.isfile(arg.twoMatrix):
            ...
            diffDbc(arg.fristMatrix, arg.twoMatrix, arg.config,arg.resultPath, '-u' in sys.argv)
    elif '-t' in sys.argv:
        repalceFirtDbcSigName(arg.fristMatrix,arg.twoMatrix,arg.config)
    elif '-d' in sys.argv:

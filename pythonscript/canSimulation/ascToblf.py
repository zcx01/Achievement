...
from can.io.generic import TextIOMessageReader, TextIOMessageWriter
...
class ASCReader7(TextIOMessageReader):
    """
    ...
    def __iter__(self) -> Generator[Message, None, None]:
        ...
        for _line in self.file:
            ...
            channel, can_id_str,direction, rest_of_message = rest_of_message.split(None, 3)
            if "is_fd" not in msg_kwargs:
                msg = self._process_classic_can_frame(rest_of_message, msg_kwargs)
            else:
                rest_of_message = f"{channel} {direction} {can_id_str} {rest_of_message}"
...
class C_ASCWriter(TextIOMessageWriter):
...
#     asc2blf(blf_file_path, asc_file_path)


def asc2blf(asc_file_path,blf_file_path):
    if not os.path.exists(asc_file_path):
        print(f'{asc_file_path} not exist!')
        return
    
    if blf_file_path == "":
        blf_file_path = os.path.splitext(asc_file_path)[0] + '.blf'
    if os.path.isdir(blf_file_path):
        blf_file_path = blf_file_path + '/' + os.path.splitext(os.path.basename(asc_file_path))[0] + '.blf'

    print(f'blf_file_path is {blf_file_path}')
    with open(asc_file_path, 'r') as f_in:
        print(f_in)
        log_in = ASCReader7(f_in)

        with open(blf_file_path, 'wb') as f_out:
...
if __name__ == "__main__":
    ...
    修改文本报警的配置文件
    ''')
    parse.add_argument('-a', '--asc_file_path', help='asc file path',type=str)
    parse.add_argument('-b', '--blf_file_path', help='blf file path',type=str,default="",nargs='?')
    arg = parse.parse_args()
    asc2blf(arg.asc_file_path,arg.blf_file_path)